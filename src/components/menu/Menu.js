...
const Menu = () => {
    ...
    useEffect(() => {
        const fetchMenuList = async () => {
            try {
                const response = await axios.get('https://restaurant-472a5-default-rtdb.firebaseio.com/menu.json');
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        if (name && ingredients && category && amount && img) {
            const item = {
                name,
                ingredients,
                category,
    ...
    const handleDeleteClick = async (id) => {
        try {
            await axios.delete(`https://restaurant-472a5-default-rtdb.firebaseio.com/menu/${id}.json`);
        ...
        <div className='container'>
            <form onSubmit={handleFormSubmit} className='form'>
                ...
                <select
                    ...
                    className='select'
                >
                    <option value='appetizer' className='option'>Appetizer</option>
                    <option value='main courses' className='option'>Main Courses</option>
                    <option value='desserts' className='option'>Desserts</option>
                </select>
                ...
                <label htmlFor='amount' className='label'>Price</label>
                <input
                    id='amount'
                    type='number'
                    className='input'
                ...
                <button type='submit' className='btn-m'>Submit</button>
            </form>
                    ...
                    {menuList.map((item) => (
                        <li key={item.id} className="list">
                            <div>
                                ...
                                <p className="value">{`Image: ${item.img}`}</p>
                            </div>
                            <div>
                                <button
                                    className="edit"
